...
export class SocketMessageAdapter implements ISocketMessageAdapter {
  ...
  private setupHandlers(): void {
    this.socketServer.setJoinRoomHandler((roomId: string, userId: string) => {
      console.log(`[Handler] User ${userId} attempting to join room ${roomId}`);
      return { roomLink: `http://localhost:3000?room=${roomId}` };
    });

    this.socketServer.setMessageHandler((roomId: string, data: { userId: string; content: string }) => {
      try {
    ...
    this.socketServer.setLeaveRoomHandler((roomId: string, userId: string) => {
      console.log(`[Handler] User ${userId} left room ${roomId}`);
    });
  }
